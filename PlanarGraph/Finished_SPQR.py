...
epsilon_tolerance = 1e-12 # * math.sqrt(amount_of_elements)
max_iters = 1e14
circle_count = 12
...
def spawn_in_area(G, ignore_list):
    ...
    for x in ignore_list:
        SV.pop(x)
    E = [(a,b) for a, b in E if not (a in ignore_list or b in ignore_list)]
    for x in ignore_list:
        E = [(a if a < x else a-1,
            b if b < x else b-1)
             for a, b in E]
...
def delaunay_triangulation(pointList):
    ...
    def inside_circumcircle(point, triangle):
        ...
        Ux_ = width if D_ == 0 else 1/D_ * (Cy_ * ( Bx_ ** 2 + By_ **2 ) - By_ * (Cx_ ** 2 + Cy_ ** 2))
        Uy_ = height if D_ == 0 else 1/D_ * (Bx_ * ( Cx_ ** 2 + Cy_ **2 ) - Cx_ * (Bx_ ** 2 + By_ ** 2))
...
# Source: https://apps.dtic.mil/sti/pdfs/AD0746856.pdf
# FINDING THE TRICONNECTED COMPONENTS OF A GRAPH

# v -> w   :  (v,w) is an edge of T
# v -*> w  :  there is a path from v to w in T
# v -> w   :  v is father, and w is son
# v -*> w  :  v is an ancestor, and w is a descedeant
# D(v)      :  set of decendants of v

# lowest vertex reachable from v
# lowpt1(v) = min({v} U {w | v -*> - -> w})

# second lowest vertex reachable from v
...
def step1():
    ...
    i = 0
    # vertex u is the father of vertex v in the spanning tree being constructed
    def search1(v,u):
        ND[v] = 0
        ...
        for w in adj[v]:
            ...
            elif number[w] < number[v] and w != u or flag[v]+false:
    ...
    for v in range(V):
        flag[v] = True
        # flag[v] becomes false when edge u -> v is traversed backwards for the first time
    search1(s, 0) # s is an arbitrary strating vertex

# step2
def sort():
    for (v,w) in P:
        if w in arrow[v]:
            bucket[2*w+1].append((v,w))
        elif lowpt2[w] < v:
            bucket[2*lowpt1[w]].append((v,w))
        else:
            bucket[2*lowpt1[w]+1].append((v,w))
    for i in range(v):
        adjecency_list[v] = [] # construct empty adjecency list for vertex v
    for i in range(2*v+1):
        for (v,w) in bucket[i]:
            adjecency_list[v].append(w)
...
def search3():
    def search2(v):
        ...
        if son1[v] == 0:
            ...
            highpt[father[w]] = max(highpt[father[w]], number[V])
            if number[w] < lowpt1[v]:
                lowpt2[v] = lowpt1[v]
                lowpt1[v] = number[w]
            elif number[w] > lowpt1[v]:
    ...
    for i in range(V):
        son1[v] = 0
    i = V
    search2(r); # r is the root of P

def tricon(v):
    for w in adjecency_list[v]:
        if w in arrow[v]: # v -> w
            tricon(w)
            # program has just return along tree arch v -> w
            edge_stack.append((v,w)) # add v->w to edge stack

            # test for type 1 component;
            if ((lowpt2[w] >= v) and ((lowpt1[w] != 1) or (degree[v] > 2) or (father[v] != lowpt1[v]))):
                j = j+1
                while edge_stack[-1][0] >= w: # while (x,y) on top of edge stack has x >= w
                    (x,y) = edge_stack.pop()

                    new_component.append((x,y)) # add (x,y) to new component
                    degree[x] -= 1
                    degree[y] -= 1
                new_component.append((v,lowpt1[w],j)) # add (v,lowpt1[w], j) to new component

                # output new component
                components.append(new_component)
                new_component = []
                # test for multiple edge
                if edge_stack[-1] == (v,lowpt1[w]):
                    j = j+1

                    new_component.append(edge_stack[-1])
                    new_component.append((v,LOWPTl(w),j-1))
                    new_component.append((v,LOWPTI(w),j))
                ...
                edge_stack.append((v,lowpt1[w], j))
                degree[v] += 1
                degree[lowpt1[w]] += 1

            # test for degree 2 vertex (component will be of type 2)
            if (degree[w] == 2) and ((v != 1) or (degree[edge_stack[-2][1]] > 2)):
                j = j+1
                new_component.append(edge_stack[-2])
                new_component.append(edge_stack[-1])
                if edge_stack[-1] == (x,v):
                    j = j+1
                    new_component.append(edge_stack[-1])
                    new_component.append((x,v,j-1))
                    new_component.append((x,v,j))
                    # output new component
                    components.append(new_component)
                    new_component = []
                    degree[v] -= 1
                ...
                edge_stack.append((x,v, j))
                father[x] = v
                if son1[v] == w:
                    son1[v] = x

                # test for type 2 pair
                while triple_stack[-1][1] == father[triple_stack[-1][2]]:
                    triple_stack.pop() # delete (h,a,b) from triple_stack

                while v == triple_stack[-1][1] and triple_stack[-1][1] != 1:
                    j = j+1
                    triple_stack.pop() # delete (h,a,b) from triple_stack
                    while a <= edge_stack[-1][0] <= h and a <= edge_stack[-1][1] <= h:
                        if edge_stack[-1] == (a,b):
                            flag = True
                            saved_edges.append((x,y)) # save edge (x,y)
                        else:
                            (x,y) = edge_stack.pop() # delete (x,y) from edge_stack and add to new component
                            new_component.append((x,y))

                            degree[x] -= 1
                            degree[y] -= 1
                        new_component.append((a,b,j))
                        if flag == True:
                            flag = False
                            j = j +1
                            new_component.append((x,y))
                            new_component.append((a,b,j-1))
                            new_component.append((a,b,j))
                            # output new component
                            components.append(new_component)
                            new_component = []

                    edge_stack.append((a,b,j))
                    degree[a] -= 1
                    degree[b] -= 1
                    # fix up triplestack
                    while .. # TODO...p.46
...
def draw_any_planar_graph(i_vertices, i_edges):
    global resMap
    step1()
    sort() # step2
    step3()
...
V,E = (list(range(16)),[(0,1),(1,7),(7,6),(6,0),(0,2),(1,3),(7,5),(6,4),(2,3),(3,5),(5,4),(4,2),(7,13),(13,12),(12,9),(9,7),(7,14),(13,14),(14,15),(15,9),(15,12),(9,8),(9,10),(9,11),(11,8),(11,10),(8,10),(8,6)])

draw_any_planar_graph(len(V), E)
