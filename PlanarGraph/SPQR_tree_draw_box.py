...
def trev(v, x, y, w, h):
    ...
    # print_filled_square(x, y, w, h, (255,(v * 80 % 255),(v * 30 % 255)))

    et += edge
    if t == "S":
        for i, vi in enumerate(vert):
            xpos, ypos = box_xy_from_i(i, (len(vert)+1) // 4)
            xpos *= w // int(math.sqrt(len(vert)))
            ypos *= h // int(math.sqrt(len(vert)))
        ...
        for i, (vix, viy) in sub_tree:
            ...
            v_res, e = trev(i, (b_x + a_x) // 2, (b_y + a_y) // 2, (b_x - a_x) // 2, (b_y - a_y) // 2)
            et += e
            vt = vt | v_res
            index += 1
    elif t == "R":
        for i, vi in enumerate(vert):
            if vi not in vt:
                x_rand = random.randint(x, x + w)
                y_rand = random.randint(y, y + h)
                vt[vi] = (x_rand, y_rand)
    elif t == "P":
        if vert[0] not in vt:
            vt[vert[0]] = x, y
        ...
        for j, (i, (vix, viy)) in enumerate(sub_tree):
            ...
            v, e = trev(i, a_x, a_y, b_x, b_y)
            vt = vt | v

    return vt, et

vertices, edges = trev(0, 0, 0, width - point_width, height - point_height)
print ("Vert", vertices)
