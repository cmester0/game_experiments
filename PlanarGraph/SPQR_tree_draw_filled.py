...
def trev(v, x, y, w, h):
    ...
    vt[v] = (x + w // 2, y + h // 2)
    print_square(x + w // 2 - 5, y + h // 2 - 5, x + w // 2 + 5, y + h // 2 + 5, (255,0,0))
    et += edge

    box_size = (len(sub_tree)+3) // 4 # Round up

    for i, (j, (vix, viy)) in enumerate(sub_tree):
        xpos, ypos = box_xy_from_i(i, box_size)
        ...
        print_square(xpos+1,ypos+1,xpos + n_w - 1,ypos + n_h - 1,(255, (v * 80) % 255,(v * 30) % 255))
        print (j)
        trev(j, xpos, ypos, n_w, n_h)
