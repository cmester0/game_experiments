...
def trev(v, polygon):
    ...
    # print_polygon(polygon, (255,(v * 80 % 255),(v * 30 % 255)))
    ...
    if t == "S":
        ...
        if len(known) == 0:
            ...
            cx = sum(x[0] for x in polygon) // len(polygon) # width // 2
            cy = sum(x[1] for x in polygon) // len(polygon) # height // 2

            nn = next(iter(polygon))
            r = math.sqrt((cx - nn[0]) ** 2 + (cy - nn[1]) ** 2) // 2

        else:
            cx = sum([vt[v][0] for i, v in known]) // len(known)
            cy = sum([vt[v][1] for i, v in known]) // len(known)

            nn = vt[next(iter(known))[1]]
            r = math.sqrt((cx - nn[0]) ** 2 + (cy - nn[1]) ** 2) // 2

        for i, v in unknown:
            phi = 2 * math.pi * i / len(vert)
            xpos = cx + math.cos(phi) * r
            ypos = cy + math.sin(phi) * r
            vt[v] = (int(xpos), int(ypos))
    elif t == "R":
        for i, v in enumerate(vert):
            if not v in vt:
                mn = min(polygon)
                mx = max(polygon)
                x_rand = random.randint(min(mn[0], mx[0]), max(mn[0], mx[0]))
                y_rand = random.randint(min(mn[1], mx[1]), max(mn[1], mx[1]))
                vt[v] =  (x_rand, y_rand)

    for j, (via, vib) in sub_tree:
        a_x, a_y = vt[via]
        b_x, b_y = vt[vib]
        v_res, e = trev(j, [(a_x, a_y), (b_x, b_y)])
