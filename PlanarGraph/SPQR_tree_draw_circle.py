...
SPQR_tree[1] = ("S", [5,6,7], [(5,(0,5)),(6,(5,6)),(7,(6,7))], [(7,1)])
SPQR_tree[5] = ("Q", [0, 5], [], [(0,5)])
SPQR_tree[6] = ("Q", [5, 6], [], [(5,6)])
SPQR_tree[7] = ("Q", [6, 7], [], [(6,7)])

SPQR_tree[2] = ("S", [1,8,2], [(8,(1,8)), (9, (8, 9))], [(9,2)])
SPQR_tree[8] = ("Q", [1,8], [], [(1,8)])
SPQR_tree[9] = ("Q", [8,2], [], [(8,9)])

SPQR_tree[3] = ("P", [2, 3], [(10, (2,3)), (11, (2,3)), (12, (2,3))], [(2,3)])

SPQR_tree[10] = ("S", [2, 9, 3], [(13, (2,9)), (14, (9,3))], [(2,3)])
SPQR_tree[13] = ("Q", [2, 9], [], [(2,9)])
SPQR_tree[14] = ("Q", [9, 3], [], [(9,3)])

SPQR_tree[11] = ("Q", [2, 3], [], [(2,3)])

SPQR_tree[12] = ("S", [2, 10, 3], [(15, (2,10)), (16, (10,3))], [(2,3)])
SPQR_tree[15] = ("Q", [2, 10], [], [(2,10)])
SPQR_tree[16] = ("Q", [10, 3], [], [(10,3)])
...
def pos_from_xyad(x,y,a,d):
    xpos, ypos = math.cos(a), math.sin(a)
    xpos = int(x + xpos * ((d) * (width - point_width) / 2) / (2 * max_depth))
    ypos = int(y + ypos * ((d) * (height - point_height) / 2) / (2 * max_depth))
    return xpos, ypos

def trev(v, x, y, a, b, d):
    global vt
    global et
    ...
    for i, e in enumerate(edge):
        for j in range(2):
            if e[j] not in vt:
                vt[e[j]] = pos_from_xyad(x,y,a + (i * 2 + j) * (b - a) / (2 * len(edge)),d)

    et += edge

    for i, (j, (vix, viy)) in enumerate(sub_tree):
        curr_a = a + i * (b - a) / len(sub_tree) # - d * 0.1
        n_a = a + (i+1) * (b - a) / len(sub_tree) # - d * 0.1

        xpos, ypos = pos_from_xyad(x,y,curr_a,d)

        c = colorsys.hsv_to_rgb(d / max_depth,1.0,1.0)

        print_square(xpos+8,ypos+8,xpos-8,ypos - 8,(int(c[0] * 255),int(c[1] * 255),int(c[2]  *255)))

        trev(j, xpos, ypos, curr_a, n_a, d+1)

    return vt, et

print_square(0,0,width - point_width, height - point_height, (255,255,255))

vertices, edges = trev(0, (width - point_width) //2, (height - point_height) //2, 0, 2 * math.pi, 0)
print (edges)
