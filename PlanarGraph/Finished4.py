...
def draw_any_planar_graph(i_vertices, i_edges):
    ...
    def find_face(edges):
        for (a,b) in edges:
            for (c,d) in edges:
                if (a == c) and (b != d) and ((b,d) in edges or (d,b) in edges):
                    return (a,b,d)
                if (a == d) and (b != c) and ((b,c) in edges or (c,b) in edges):
                    return (a,b,c)
                if (b == c) and (a != d) and ((a,d) in edges or (d,a) in edges):
                    return (a,b,d)
                if (b == d) and (a != c) and ((a,c) in edges or (c,a) in edges):
                    return (a,b,c)
        print ("Could not find face")
        exit(0)

    V, E = list(range(i_vertices)), list(i_edges)

    print ("Random map A")
    V_, E_ = add_color(([random_point() for _ in range(i_vertices)], E))
    print_graph_scaled((V_, E_))
    draw_and_reset('finished_a_data.png')

    outer_list = find_face(i_edges)
    p, E_ = ForceDirected((V,E), epsilon_tolerance, max_iters, outer_list, [(int((width * 4) * math.cos(phi) + width/2), int((height * 4) * math.sin(phi) + height/2))
                                                                            for phi in [math.pi / 180 * i for i in range(0,360,360//len(outer_list))]])
    (V, E) = (list(p), list(E_))

    print ("Random map B")
    V_, E_ = add_color((V, E))
    print_graph_scaled((V_, E_))
    draw_and_reset('finished_b_data.png')

    outer_circle = [
        (int((width * 8) * math.cos(phi) + width/2),
         int((height * 8) * math.sin(phi) + height/2)) for phi in [math.pi / 180 * i for i in range(0,360,360//circle_count)]]
    plist = outer_circle + [(x + (random.randint(0,1000) - 500) / 10, y + (random.randint(0,100) - 50) / 10) for x,y in V]

    print ("Random map C")
    V_, E_ = add_color((plist, E))
    print_graph_scaled((V_, E_))
    draw_and_reset('finished_c_data.png')
    ...
    V_, E_ = add_color((V, E))
    print_graph_scaled((V_, E_))
    draw_and_reset('finished_d_data.png')
    ...
    print ("Random map")
    V_, E_ = add_color((V, i_edges))
    print_graph_scaled((V_, E_))
    draw_and_reset('finished_e_data.png')
